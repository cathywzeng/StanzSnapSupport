...
StanzSnap 是一款智能相机应用，通过为不同场景应用最佳相机设置来帮助用户拍出更好的照片。

**我们不收集、存储或共享任何个人数据。**

#### 我们不收集的内容
- 位置数据（尽管 iOS 可能请求此权限，但我们不会访问或使用它）
- 照片或相机内容（所有处理均在设备上完成；照片仅保存到您的本地相册）
- 使用分析或崩溃报告
- 用于广告或追踪的设备标识符

#### 我们使用的权限
- **相机**：用于拍照
- **相册（保存）**：用于将处理后的照片保存到您的相册
- **位置**：由于第三方库的要求在 Info.plist 中声明，但我们不会访问或收集位置数据

#### 第三方服务
我们不使用任何第三方分析、广告或数据收集服务。

#### 数据存储
所有应用设置均使用加密存储（`expo-secure-store`）保存在您的设备本地。没有任何数据会传输到外部服务器。

#### 儿童隐私
StanzSnap 不面向 13 岁以下儿童。我们不会故意收集来自儿童的任何信息。

#### 政策变更
我们可能会不时更新本隐私政策。任何更改将发布在本页面。
